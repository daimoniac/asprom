<h2>Edit Schedule for {{iprange}}</h2>
...
$(document).ready(function() {
	...
	$('#submitt').click(function() {
		...
		$.ajax({
			url: "/controller/editjob/{{jobid}}",
			type: "POST",
			cache: false,
			data: dataVal,
			success: function( json ) {
				addAlert("Applied changes to Job ID {{jobid}}", 'success', true);
				$('#datatable').bootstrapTable('refresh');
				BootstrapDialog.closeAll();
			},
			error: function( xhr, status, errorThrown ) {
				addAlert("Error submitting changes to Job ID {{jobid}}", 'danger');
			},
		});
	});
...
<div class="input-group">
  ...
  <input type="text" class="form-control" id="iprange" value="{{iprange}}">
</div>
<br/>
<div class="input-group">
  <span class="input-group-addon">Port Range</span>
  <input type="text" class="form-control" id="portrange" value="{{portrange}}">
</div>
<br/>
<div class="input-group">
  <span class="input-group-addon">extra Parameters</span>
  <input type="text" class="form-control" id="extraparams" value="{{extraparams}}">
</div>
<br/>
<!---
<div class="input-group">
...
<button id="submitt" class="btn btn-default">Apply</button>
