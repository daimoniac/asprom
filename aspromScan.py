...
>        usage:
    ...
    aspromScan.py [-h] [-o EXTRA_OPTIONS] [-s SENSOR] [-p PORT_RANGE] [-j JOB_ID] TARGET
    ...
    '''
    parser = argparse.ArgumentParser(description='Scans an IP Range for asprom. Needs nmap installed on the sensor host.')
...
def __targetFormat(v):
    ...
    try:
        #cidr range
        return re.match("^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])(\/(\d|[1-2]\d|3[0-2]))?$", v).group(0)
    except:
        try:
            #hostname
            return re.match("^(([a-zA-Z]|[a-zA-Z][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z]|[A-Za-z][A-Za-z0-9\-]*[A-Za-z0-9])$", v).group(0)
        except:
